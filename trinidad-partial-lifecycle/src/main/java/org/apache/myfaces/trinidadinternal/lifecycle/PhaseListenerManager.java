...
class PhaseListenerManager
{
  ...
  private static final TrinidadLogger LOG = TrinidadLogger.createTrinidadLogger(PhaseListenerManager.class);
    ...
    for (int i = 0; i < phaseListeners.length; i++)
    {
      ...
      if (isListenerForThisPhase(phaseListener, phaseId))
      {
        ...
        } catch (Exception e)
        {
          ...
          LOG.severe("Exception in PhaseListener " + phaseId.toString() + " beforePhase.", e);
          return;
        }
      }
    }
  ...
  void informPhaseListenersAfter(PhaseId phaseId)
  {
    ...
    for (int i = phaseListeners.length - 1; i >= 0; i--)
    {
          ...
          && beforePhaseSuccess[i])
      {
        ...
        {
          LOG.severe("Exception in PhaseListener " + phaseId.toString() + " afterPhase", e);
        }
      }
    }
  }
